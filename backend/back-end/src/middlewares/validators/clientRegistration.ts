...
const validateForm = async (
  ...
  try {
    ...
    if (gstNumber) {
      const company = await Client.findOne({ gstNumber: gstNumber });
      console.log(company);
      if (company) {
        throwError('Company is already registered', 400);
      } else {
        ...
        if (!companyName) {
          throwError('Please enter company name', 404);
        } else {
          ...
          if (!validator.isFloat(yearsOfIncorporation, { min: 0, max: 25 })) {
            throwError(
              'Please enter years of incorporation between 0 to 25',
              400
            );
          }
          ...
          if (!validator.isFloat(cgstPercentage, { min: 0, max: 50.0 })) {
            throwError('Please enter cgst percentage between 0 to 50', 400);
          }
          if (!validator.isFloat(sgstPercentage, { min: 0, max: 50.0 })) {
            throwError('Please enter sgst percentage between 0 to 50', 400);
          }
